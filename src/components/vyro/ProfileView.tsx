...
export function ProfileView() {
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        eyebrow="Account · scout profile"
        title={name || "Your profile"}
        subtitle="Identity, sport, settings, and the band you're paired with."
      />
      ...
      <Card eyebrow="Public scout profile" title="Recruiter view">
        ...
        <p className="mt-3 rounded-xl border border-vyro-line bg-vyro-elev p-3 text-[12px] text-vyro-mute">
          <span className="font-mono text-[9px] uppercase tracking-wider text-vyro-text">
            Coach note ·{" "}
          </span>
          Average metric breakdown and heat-map movement are shared in the recruiting / coach
          packet.
        </p>
        <label className="mt-3 flex items-center gap-2 text-[12px] text-vyro-text">
          <input
            type="checkbox"
            checked={scoutPublic}
            onChange={(e) => setScoutPublic(e.target.checked)}
            className="h-4 w-4 accent-[var(--vyro-mint)]"
          />
      ...
      <Card eyebrow="Safety + privacy" title="Coach access, emergency contact, prototype notes">
        ...
        <Field label="Data sharing">
          <Select
            value={sharing}
            onChange={(v) => setSharing(v as DataSharing)}
            options={[
        ...
        <Field label="Coach visibility">
          <Select
            ...
            onChange={(v) => setCoachVisibility(v as Privacy)}
            options={[
              ["private", "Private"],
              ["team", "Team + coach"],
              ["coach", "Coach only"],
      ...
      <Card eyebrow="App settings" title="Display + defaults">
        ...
        <Field label="Metric detail default">
          <Select
            value={metricDetail}
            onChange={(v) => setMetricDetail(v as MetricDetail)}
            options={[
              ["expanded", "Expanded on tap"],
              ["compact", "Compact"],
              ["coach", "Coach-level detail"],
              ["beginner", "Beginner explanations"],
            ]}
          />
        </Field>
