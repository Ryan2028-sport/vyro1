...
export function AthleteView() {
  ...
  const guidance = useMemo(() => {
    ...
    if (band === "red")
      return "Recovery is low. Skip ghosting drills, choose mobility + light hitting.";
    if (band === "yellow")
      return "Moderate readiness — technique work and 2 short interval blocks max.";
    if (sessionLoad != null && sessionLoad > 70)
      return "You're already deep in load. Cool down and rehydrate.";
  ...
  return (
    <div className="space-y-4">
      ...
      <Card
        eyebrow="Court readiness"
        title="Today's status"
        action={<Pill tone={bandTone}>{bandLabel}</Pill>}
      >
      ...
      {!m.connected && (
        <Card>
          <p className="text-[12px] leading-relaxed text-vyro-mute">
            <span className="text-vyro-text font-bold">Band offline.</span> Pair and wear your VYRO
            Band to stream HR, HRV, SpO₂, skin temp, steps and IMU load in real time. Values below
            stay blank until live data arrives — nothing is estimated.
          </p>
        </Card>
      )}
      ...
      {section === "overview" && (
        <Card eyebrow="At a glance" title="Right now">
          <div className="grid grid-cols-2 gap-2">
            ...
            <Stat
              label="Battery"
              value={live(m.batteryPct) ?? "—"}
              unit="%"
              hint={m.connected && m.batteryCharging ? "charging" : undefined}
            />
          </div>
      ...
      {section === "cardiac" && (
        <Card eyebrow="Heart · every 1 s" title="Cardiac">
          <div className="grid grid-cols-2 gap-2">
            <Stat
              label="Current HR"
              value={live(m.heartRateBpm) ?? "—"}
              unit="bpm"
              hint={live(m.heartRateBpm) != null ? "live" : undefined}
            />
            <Stat
              label="Resting HR"
              value={live(m.restingHrBpm) ?? "—"}
              unit="bpm"
              hint={live(m.restingHrBpm) != null ? "rolling live HR" : undefined}
            />
            <Stat
              label="HRV (RMSSD)"
              value={live(m.hrvMs) ?? "—"}
              unit="ms"
              hint={live(m.hrvMs) != null ? "watch frame" : undefined}
            />
            <Stat
              label="Stress"
              value={live(m.stressScore) ?? "—"}
              hint={live(m.stressScore) != null ? "watch stress frame" : undefined}
            />
            <Stat
              label="Blood pressure"
              value={
                live(m.bloodPressure)
                  ? `${live(m.bloodPressure)!.sbp}/${live(m.bloodPressure)!.dbp}`
                  : "—"
              }
              unit="mmHg"
            ...
            <Stat
              label="Resp rate"
              value={live(m.respRateBrpm) ?? "—"}
              unit="br/min"
              hint={live(m.respRateBrpm) != null ? "watch field" : undefined}
            />
          </div>
        </Card>
      )}
        ...
        <>
          ...
          <Card eyebrow="Court fueling" title="Hydration & energy">
            <p className="text-[11px] leading-relaxed text-vyro-mute">
              Hydration and energy guidance unlock once live HR, skin temp and session-load frames
              stream continuously. Nothing appears here from cached or estimated body signals.
            </p>
          </Card>
        </>
      )}
      ...
      {section === "load" && (
        <>
          <Card
            ...
            action={
              <Pill tone={(sessionLoad ?? 0) > 70 ? "warn" : "live"}>
                {sessionLoad == null ? "—" : `${sessionLoad}/100`}
              </Pill>
            }
          >
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              <Stat
                label="Events / min"
                value={m.connected ? m.eventsLastMin : "—"}
                hint="rolling 60 s"
              />
              <Stat label="Peak accel" value={m.connected ? m.peakG.toFixed(2) : "—"} unit="g" />
              <Stat
                label="Peak jerk"
                value={m.connected ? m.peakJerk.toFixed(0) : "—"}
                unit="g/s"
              />
              <Stat
                label="Reaction"
                value={m.connected && m.reactMin != null ? m.reactMin.toFixed(0) : "—"}
                unit="ms"
              />
            </div>
          </Card>
          <Card eyebrow="Activity · all-day" title="Movement">
            <div className="grid grid-cols-2 gap-2">
              ...
              <Stat
                label="Distance"
                value={live(m.distanceM) != null ? (live(m.distanceM)! / 1000).toFixed(2) : "—"}
                unit="km"
              />
          ...
          <Card eyebrow="Injury risk · squash" title="Hot spots">
            {m.connected ? (
              <div className="space-y-2">
                <RiskRow
                  zone="Lunging knee"
                  level={band === "red" ? "elevated" : "low"}
                  note="Front-foot decel load"
                />
                <RiskRow
                  zone="Achilles / calf"
                  level={(sessionLoad ?? 0) > 70 ? "elevated" : "low"}
                  note="Repeated push-off"
                />
