...
function saveBuf(b: Sample[]) {
  if (typeof window === "undefined") return;
  try {
    window.localStorage.setItem(BUF_KEY, JSON.stringify(b.slice(-BUF_MAX)));
  } catch {
    /* quota */
  }
}
...
function computeSleepScore(samples: Sample[]): {
  score: number | null;
  cardiac: number | null;
  ...
  const cardiac =
    hrs.length >= 3 && min(hrs) != null
      ? Math.round(clamp(((75 - (min(hrs) as number)) / 35) * 100))
      : null;
  const recovery =
    hrvs.length >= 3 && avg(hrvs) != null
...
export function SleepView() {
  ...
  const { score, cardiac, recovery, thermal } = useMemo(
    () => computeSleepScore(samples),
    [samples],
  );
  ...
  return (
    <div className="mx-auto max-w-[430px] space-y-5 pb-8 text-vyro-text">
      <header className="space-y-2">
        ...
        <h2 className="text-[26px] font-black leading-[1.06] tracking-[-0.04em] text-vyro-text">
          Live sleep score
        </h2>
        <p className="max-w-[390px] text-[13px] leading-relaxed text-vyro-mute">
          Derived only from BLE heart-rate, HRV and skin-temperature samples collected between 10pm
          and 8am. The firmware does not emit sleep stages, so no hypnogram, no wakeups, no debt
          curves are shown.
        </p>
        <span className="inline-flex min-h-9 items-center gap-2 rounded-lg border border-vyro-text/35 bg-vyro-text/5 px-3 font-mono text-[9.5px] font-medium uppercase tracking-[0.14em] text-vyro-text">
          ...
          {connected
            ? "Band live · sampling"
            : samples.length
              ? "Last overnight window"
              : "Awaiting overnight data"}
        </span>
      </header>
      ...
      <VCard>
        <div className="flex items-center justify-between">
          <p className="font-mono text-[9px] font-medium uppercase tracking-[0.24em] text-vyro-mute">
            Overnight window
          </p>
        ...
        <p className="mt-4 text-[11px] leading-relaxed text-vyro-mute">
          Sample count in window: <span className="font-mono text-vyro-text">{samples.length}</span>{" "}
          · Rolling buffer: <span className="font-mono text-vyro-text">{bufSize}</span>
        </p>
      </VCard>
      ...
      <VCard>
        ...
        <ul className="mt-3 space-y-2 text-[12px] leading-relaxed text-vyro-mute">
          <li>
            <span className="text-vyro-text">Cardiac calm (45%):</span> lower min HR overnight
            scores higher.
          </li>
          <li>
            <span className="text-vyro-text">Recovery (40%):</span> higher average HRV scores
            higher.
          </li>
          <li>
            <span className="text-vyro-text">Thermal stability (15%):</span> lower skin-temp
            variance scores higher.
          </li>
          <li>
            Weights renormalise over whichever channels actually have data — missing channels are
...
function ScoreCard({
  ...
  return (
    <VCard className="border-vyro-line">
      ...
      {samples < 3 && (
        <p className="mt-4 text-[11px] leading-relaxed text-vyro-mute">
          Need at least 3 overnight samples (HR/HRV/temp) to compute a score. Currently have{" "}
          <span className="font-mono text-vyro-text">{samples}</span> in the overnight window
...
function Sub({ label, value }: { label: string; value: number | null }) {
  return (
    <div className="rounded-[14px] border border-vyro-line bg-vyro-text/[0.03] p-3 transition-colors duration-200 hover:bg-vyro-text/[0.055] text-center">
      <div className="font-mono text-[8.5px] font-medium uppercase tracking-[0.15em] text-vyro-mute">
        {label}
      </div>
      <div className="mt-1 text-[21px] font-black leading-none tracking-[-0.04em] tabular-nums text-vyro-text">
        {value != null ? value : "—"}
      </div>
    </div>
  );
  ...
  return (
    <div className="rounded-[14px] border border-vyro-line bg-vyro-text/[0.03] p-3 transition-colors duration-200 hover:bg-vyro-text/[0.055]">
      ...
      <div className="mt-2 flex items-baseline gap-1">
        <span className="text-[21px] font-black leading-none tracking-[-0.04em] tabular-nums text-vyro-text">
          {value}
        </span>
...
function VCard({ children, className = "" }: { children: ReactNode; className?: string }) {
  return (
    <section
      className={`rounded-[20px] border border-vyro-line bg-vyro-panel bg-[linear-gradient(160deg,rgba(255,255,255,0.045),transparent_40%)] p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.05),0_18px_44px_-24px_rgba(0,0,0,0.9)] transition-[border-color,box-shadow] duration-200 ease-out hover:border-vyro-text/[0.12] ${className}`}
    >
      {children}
    </section>
  );
...
function SleepRing({ value }: { value: number | null }) {
  ...
  return (
    <div className="mx-auto w-full max-w-[250px]">
      <svg
        viewBox="0 0 190 190"
        className="mx-auto h-[190px] w-[190px] -rotate-90 overflow-visible"
        aria-label={`Sleep score ${value ?? "unknown"} out of 100`}
      >
      ...
      <div className="pointer-events-none -mt-[118px] flex h-[118px] flex-col items-center justify-start text-center">
        <span className="text-[36px] font-black leading-none tracking-[-0.045em] tabular-nums text-vyro-text">
          {value ?? "—"}
        </span>
        <span className="mt-2.5 font-mono text-[11px] text-vyro-mute">/ 100</span>
      </div>
      <p className="mt-2 text-center font-mono text-[8.5px] font-medium uppercase tracking-[0.24em] text-vyro-mute">
        Sleep
      </p>
    </div>
  );
